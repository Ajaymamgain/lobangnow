...
class ImagenService {
  ...
  async generateFoodImages(dealData, restaurantData, numImages = 6, options = {}) {
    try {
      console.log('[ImagenService] Starting food image generation for:', dealData.title);
      ...
      for (let i = 0; i < numImages; i++) {
        console.log(`[ImagenService] Generating image ${i + 1}/${numImages}...`);
        ...
        if (imageResult.success) {
          ...
          console.log(`[ImagenService] Image ${i + 1} generated successfully`);
        } else {
                      console.warn(`[ImagenService] Image ${i + 1} generation failed:`, imageResult.error);
        }
      }
      
      console.log(`[ImagenService] Completed food image generation. ${images.length}/${numImages} images successful`);
      ...
      console.error('[ImagenService] Food image generation failed:', error.message);
      return {
        success: false,
        error: error.message,
        images: []
  ...
  async generateSingleFoodImage(prompt, dealData, restaurantData, imageIndex, options = {}) {
    try {
      ...
      // Get the image URL - Imagen returns a direct URL string
      ...
      console.error(`[ImagenService] Single image generation failed for index ${imageIndex}:`, error.message);
      return {
        success: false,
        error: error.message,
        imageIndex: imageIndex
